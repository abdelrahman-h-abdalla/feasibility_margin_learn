...
class TrainingDataset:
    ...
    def get_training_data_parser(self, process_data=True, max_files=None):
        if self._data_parser is None:
            ...
            roll_pitch = training_data[:, :2]
            print("roll_pitch", roll_pitch[0:10])
            # Create a column of zeros for yaw
            yaw_zeros = np.zeros((training_data.shape[0], 1))
            # Concatenate roll, pitch, and yaw_zeros to form the rpy array
            rpy = np.hstack((roll_pitch, yaw_zeros))
            print("rpy", rpy[0:10])
            # Compute rotation matrix (from base to world) from roll/pitch/yaw - Intrinsic rotations.
            rotation_matrices = R.from_euler('XYZ', rpy, degrees=False).as_dcm() #
            print("rotation_matrices", rotation_matrices[0:10])
            ...
            print("training_data[0:10]", training_data[0:10])
