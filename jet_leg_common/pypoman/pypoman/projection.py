...
def project_polytope(proj, ineq, A_y, eq=None, method='cdd', **kwargs):
    """
    ...
    if method == 'bretl':
        ...
        return project_polytope_bretl(proj, ineq, eq, A_y, **kwargs)
    vertices, rays = project_polyhedron(proj, ineq, eq)
    assert not rays, "Projection is not a polytope"
    return vertices


def project_polytope_bretl(proj, ineq, eq, A_y, max_radius=1e5, max_iter=1000,
                           init_angle=None):
    """
...
def project_polytope_general_plane(ineq, eq=None, **kwargs):
    """
    Project, on a general plane of interest, the polytope defined by :
    ...
    """
    assert eq is not None, "Bretl method requires = constraints for now"
    return project_polytope_general_plane_bretl(ineq, eq, **kwargs)

    return vertices

def project_polytope_general_plane_bretl(ineq, eq, max_radius=1e5, max_iter=1000,
                                         init_angle=None):
    """
    Project a polytope into a 2D polygon using the incremental projection
    algorithm from [Bretl08]_. The 2D affine projection :math:`y = E x + f` is
    ...
    # C_ext[-2:, -2:] = array([[-1, 0], [0, -1]])
    # C_ext[:-2, -2:] = A_y
    C_ext = cvxopt.matrix(C)

    # d_ext = zeros(d.shape[0] + 2)
    # d_ext[:-2] = d
    # d_ext[-2:] = -f[:2]

    d_ext = cvxopt.matrix(d)
