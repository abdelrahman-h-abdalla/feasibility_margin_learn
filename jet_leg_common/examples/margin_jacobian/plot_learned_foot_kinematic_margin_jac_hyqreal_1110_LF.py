...
num_of_tests = 400
delta_pos_range_x = 20
delta_pos_range_y = 20
...
def plot_learnt_margin():
    ...
    for delta in delta_pos_range_vec_x:
        network_input = np.concatenate([
            ...
            np.array([-0.67, -0.6138, 0.0]),
            np.zeros(3),
            np.zeros(3),
            np.zeros(3),
            np.array([-7.8 + delta, -5.1, -0.55]),
            np.array([0.44, -0.2, -0.55]),
            np.array([3.93, 5.45, -0.55]),
            np.array([0.6]),
...
def plot_analytical_margin():
    ...
    comWF_lin_acc = np.array([-0.6704, -0.6138, .0])
    ...
    LF_foot = np.array([-7.8, -5.1, -0.55])
    RF_foot = np.array([0.44, -0.2, -0.55])
    LH_foot = np.array([3.93, 5.45, -0.55])
    ...
    foot_id = 0
    margin_foot_pos_x, jac_foot_pos_x = jac.plotMarginAndJacobianWrtFootPosition(params_foot_x, foot_id, delta_pos_range_vec_x, 0)
    margin_foot_pos_y, jac_foot_pos_y = jac.plotMarginAndJacobianWrtFootPosition(params_foot_y, foot_id, delta_pos_range_vec_y, 1)
    margin_foot_pos_z, jac_foot_pos_z = jac.plotMarginAndJacobianWrtFootPosition(params_foot_z, foot_id, delta_pos_range_vec_x, 2)
...
def main():
    ...
    delta_pos_range_vec_x = np.linspace(-7.8-delta_pos_range_x/2.0, -7.8 + delta_pos_range_x/2.0, num_of_tests)
    delta_pos_range_vec_y = np.linspace(-5.1-delta_pos_range_y/2.0, -5.1 + delta_pos_range_y/2.0, num_of_tests)
