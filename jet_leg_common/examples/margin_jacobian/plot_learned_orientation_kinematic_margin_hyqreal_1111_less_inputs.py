...
num_of_tests = 25
delta_pos_range_x = 1.4
delta_pos_range_y = 1.0
delta_pos_range_z = 1.0
...
def plot_learnt_margin():
    ...
    network = MultiLayerPerceptron(in_dim=15, out_dim=1, hidden_layers=[256,256,128], activation='relu', dropout=0.0)
    ...
    for delta in delta_orient_range_vec_x:
        ...
        # ])
        P_LF_BF = np.dot(rotation_matrix.transpose(), (np.array([0.44, 0.34, -0.4]) - np.array([0.006, -0.002, -0.044]))) + \
                    np.array([0.006, -0.002, -0.044])
        P_RF_BF = np.dot(rotation_matrix.transpose(), (np.array([0.44, -0.34, -0.4]) - np.array([0.006, -0.002, -0.044]))) + \
                    np.array([0.006, -0.002, -0.044])
        P_LH_BF = np.dot(rotation_matrix.transpose(), (np.array([-0.44, 0.34, -0.56]) - np.array([0.006, -0.002, -0.044]))) + \
                    np.array([0.006, -0.002, -0.044])
        P_RH_BF = np.dot(rotation_matrix.transpose(), (np.array([-0.44, -0.34, -0.56]) - np.array([0.006, -0.002, -0.044]))) + \
                    np.array([0.006, -0.002, -0.044])
        network_input = np.concatenate([
            rotation_matrix[2],
            # euler_angles,
        ...
        jac_array_roll.append(np.dot(gradient_g.transpose(), np.array([np.cos(rpy[0]) * np.sin(rpy[2]) + np.sin(rpy[0]) * np.cos(rpy[2]) * np.sin(rpy[1]),
                                                        np.cos(rpy[2]) * np.cos(rpy[0]) - np.sin(rpy[0]) * np.sin(rpy[1]) * np.sin(rpy[2]),
                                                        - np.sin(rpy[0]) * np.cos(rpy[1])])))
        # rotation_matrices
    ...
    for delta in delta_orient_range_vec_y:
        ...
        gradient_g = unnormalized_gradient[0:3]
        rpy = euler_angles
        jac_array_pitch.append(np.dot(gradient_g.transpose(), np.array([- np.cos(rpy[0]) * np.cos(rpy[2]) * np.cos(rpy[1]),
                                                        np.cos(rpy[0]) * np.cos(rpy[1]) * np.sin(rpy[2]),
                                                        - np.cos(rpy[0]) * np.sin(rpy[1])])))
    ...
    for delta in delta_orient_range_vec_z:
        ...
        gradient_g = unnormalized_gradient[0:3]
        rpy = euler_angles
        jac_array_yaw.append(np.dot(gradient_g.transpose(), np.array([np.sin(rpy[0]) * np.cos(rpy[2]) + np.cos(rpy[0]) * np.sin(rpy[2]) * np.sin(rpy[1]),
                                                        - np.sin(rpy[2]) * np.sin(rpy[0]) + np.cos(rpy[0]) * np.sin(rpy[1]) * np.cos(rpy[2]),
                                                     0])))


    return [[out_array_x, out_array_y, out_array_z], [jac_array_roll, jac_array_pitch, jac_array_yaw]]
...
def plot_analytical_margin():
    ...
    LF_foot = np.array([0.24, 0.34, -0.4])
    RF_foot = np.array([0.24, -0.34, -0.4])
    LH_foot = np.array([-0.64, 0.34, -0.56])
    RH_foot = np.array([-0.64, -0.34, -0.56])
