...
class Math:
    ...
    ''' 
        ...
        https: // en.m.wikipedia.org / wiki / Rotation_formalisms_in_three_dimensions?wprov = sfla1  # Rotation_matrix_.E2.86.94_Euler_angles 
        '''
    def rotToRpy(self, R):
    ...
    def vectorToRpy(self, vector):
        vector = self.normalize(vector)
        r, p, y = self.rotToRpy(self.rotation_matrix_from_normal(vector))
        return r, p, y

    def normalToRpy(self, terrain_normal):
        r, p, y = self.vectorToRpy(terrain_normal)
        y = 0.0
        return r, p, y
