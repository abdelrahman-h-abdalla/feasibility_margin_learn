...
from common.datasets_split import TrainingDataset
...
LEARNING_RATE_DEFAULT = 0.0008715165930804925
HIDEN_LAYERS_DEFAULT = [512, 256, 128]
...
def main_train(config=None, stance_legs_str=None):
    ...
    data_folder_name = 'stability_margin/1111/'
    ...
    network = MultiLayerPerceptron(in_dim=network_input_dim+1, out_dim=1, hidden_layers=config.hidden_layers, activation=config.activation_function)
...
def run_single(stance_legs_str):
    config = DotDict({
        ...
        'hidden_layers': HIDEN_LAYERS_DEFAULT,
