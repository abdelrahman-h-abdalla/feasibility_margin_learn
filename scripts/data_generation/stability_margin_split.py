...
def computation(i, q):
    ...
    com_lin_acc = np.array([0., 0., 0.])  # Random COM linear acceleration
    # com_ang_acc = angular_acceleration()  # Random COM angular acceleration
    com_ang_acc = np.array([0., 0., 0.])  # Random COM angular acceleration
    ...
    feet_contact_normals = []
    for i in range(4):
        feet_contact_normals.append(
                np.transpose(np.transpose(math_tools.rpyToRot(0., 0., 0.)).dot(np.array([0, 0, 1]))))
    # ext_force_world = base_external_force(robot_name)  # External force applied to base in world frame
    ext_force_world = np.array([0., 0., 0.])  # External force applied to base in world frame
    # ext_torque_world = base_external_torque(robot_name)
    ext_torque_world = np.array([0., 0., 0.])  # External torque applied to base in world frame
    ...
    try:
        stability_margins = compute_stability(
            comp_dyn=comp_dyn,
        ...
        stability_margin_x = stability_margins[0]
        stability_margin_y = stability_margins[1]
        ...
        # ])
        parameters = np.concatenate([
            np.array(com_euler[:2]).flatten(),
            np.array(stance_feet_pos).flatten()
        ])
        parameters = np.array(parameters, dtype=np.float)
        results = np.concatenate([
            np.array([stability_margin_x]).flatten(),
            np.array([stability_margin_y]).flatten()
